    ...
    if(rank==0){
        ...
        //double Grad=atof(digito2);
            ...
            if(cuenta==0){
                    ...
                    ficheroEntrada.getline(fra, 50);
                    //cout << "frase: " << tri->frase << endl;
            }else{
                Triangulo *tri = new Triangulo();
                ficheroEntrada.getline(fra, 50);
                //cout << tri->frase << endl;
                MPI_Send(&fra //REFERENCIA AL VECTOR DE ELEMENTOS A ENVIAR
                    ,50 //TAMAÑO del vector a enviar
                    ,MPI_CHAR //TIPO DE DATO A ENVIAR
                    ,rank+1 //PID DEL PROCESO DE DESTINO
                    ,0 //ETIQUETA
        ...
        //for(int d=0;d<9664;d++){
        Triangulo *tri = new Triangulo();
        MPI_Recv(&tri->frase //REFERENCIA AL VECTOR DONDE SE ALMACENA LO RECIVIDO
             ,50 //NUMERO DE ELEMENTOS MAXIMO ARECIVIR
             ,MPI_CHAR //TIPO DE DATO QUE RECIVE
             ,0 //PID DEL PROCESO DE ORIGEN QUE SE RECIVE
             ,0 //ETIQUETA
             ,MPI_COMM_WORLD //COMUNICADOR POR EL QUE SECIVE
